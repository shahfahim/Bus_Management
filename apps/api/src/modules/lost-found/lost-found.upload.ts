...
export const lostFoundUpload: RequestHandler = (request, response, next) => {
  fields(request, response, (error: unknown) => {
    if (!error) return next();
    if (error instanceof multer.MulterError) {
      return next(new AppError(400, 'INVALID_IMAGE_UPLOAD', error.message));
    }
    return next(error);
  });
};
...
export interface StoredLostFoundImage {
  filename: string;
  url: string;
  mimeType: string;
  originalName: string;
...
const uploadedFiles = (request: Request): Express.Multer.File[] => {
  if (!request.files || Array.isArray(request.files)) return request.files ?? [];
  return [...(request.files.image ?? []), ...(request.files.images ?? [])];
};
  ...
  try {
    for (const file of files) {
      ...
      stored.push({
        ...
        url: `${env.PUBLIC_API_URL.replace(/\/$/, '')}/api/lost-found/images/${filename}`,
      });
    }
    return stored;
  } catch (error: unknown) {
    await removeLostFoundUploads(stored);
    throw error;
  }
};
...
export const sendLostFoundImage: RequestHandler = (request, response, next) => {
  ...
  if (typeof filename !== 'string' || !/^[0-9a-f-]{36}\.(?:jpg|png|webp)$/.test(filename) || extname(filename).length > 5) {
