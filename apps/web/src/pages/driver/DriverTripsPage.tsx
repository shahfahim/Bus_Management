...
export function DriverTripsPage() {
  ...
  return (
    <div className="page-stack">
      <PageHeader actions={user?.role === 'DRIVER' ? <Link className="button button--primary button--md" to="/driver/trips/new">Set up a trip <ArrowRight aria-hidden="true" size={16} /></Link> : undefined} description="Review assignments, passenger load and real-time trip state." eyebrow="Driver operations" title="Assigned trips" />
      <Card className="driver-trip-filters"><label className="field"><span className="field__label">Service date</span><input onChange={(event) => setDate(event.target.value)} type="date" value={date} /></label><SelectField label="Trip status" onChange={(event) => setStatus(event.target.value)} options={[{ value: '', label: 'All statuses' }, { value: 'SCHEDULED', label: 'Scheduled' }, { value: 'BOARDING', label: 'Boarding' }, { value: 'IN_PROGRESS', label: 'In progress' }, { value: 'DELAYED', label: 'Delayed' }, { value: 'COMPLETED', label: 'Completed' }, { value: 'CANCELLED', label: 'Cancelled' }]} value={status} /><Button onClick={() => void load()} variant="secondary">Refresh assignments</Button></Card>
